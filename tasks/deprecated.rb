...
class Deprecated < Thor
  ...
  def vault_encrypt
    say I18n.t(:s_deprecated_encrypting).yellow
    begin
      execute_in_shell("ansible-vault encrypt #{options[:config_dir]}/vault.yml")
      say I18n.t(:s_deprecated_encrypted).green
    rescue Subprocess::NonZeroExit
      say I18n.t(:s_deprecated_failencrypt).green
    end
  end

  desc 'vault_decrypt', 'Decrypts the ansible vault.yml - DEPRECATED'
  def vault_decrypt
    say I18n.t(:s_deprecated_decrypting).yellow
    begin
      execute_in_shell("ansible-vault decrypt #{options[:config_dir]}/vault.yml")
      say I18n.t(:s_deprecated_decrypted).green
    rescue Subprocess::NonZeroExit
      say I18n.t(:s_deprecated_faildecrypt).green
    end
  end
end
